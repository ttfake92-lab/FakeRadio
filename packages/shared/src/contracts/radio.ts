...
export const SettingsSchema = z.object({
  ...
  ttsProvider: z.enum(["grok", "mimo", "fish"]).default("grok"),
  ttsVoice: z.string().min(1).default("eve"),
  mimoVoice: z.string().min(1).default("茉莉"),
  // Fish Audio voice model ID(fish.audio 音色页面复制)。空串 = 未配置,provider 切到 fish 前需填写。
  fishVoiceId: z.string().trim().max(100).default(""),
...
export const UpdateSettingsRequestSchema = z.object({
  ...
  ttsProvider: z.enum(["grok", "mimo", "fish"]).optional(),
  ttsVoice: z.string().min(1).optional(),
  mimoVoice: z.string().min(1).optional(),
  fishVoiceId: z.string().trim().max(100).optional(),
