...
- 歌曲音频需服务端录制：音乐当前在浏览器端流式播放，要生成完整音频需在服务端重新拉取音频流并写盘。
- 音频混音依赖 FFmpeg：TTS + 压低配乐 + 完整歌曲的拼接需要 FFmpeg。
- 导出为本地文件，不涉及自动分发到第三方平台。
- 音频导出仅供个人分享，非商业发布。
...
| A-07 | 对话会话持久化 | AFK |
| A-08 | 日终品味推断 | AFK |
| A-09 | 播放时服务端录制音频流 | HITL |
| A-10 | 音频混音引擎 | AFK |
| A-11 | Show notes 生成器 | AFK |
| A-12 | 导出流水线 | AFK |
