...
### 1. mock / 本地用户流仍会触发真实公开资料 adapter
...
### 2. ProgramBrief 生命周期没有进入 completed / failed 终态
...
- [ ] 本地 mock / 默认测试路径不会创建真实 public metadata 或 web research provider；没有 API key 时应使用 mock / disabled adapter，并在 trace 中记录摘要级 fallback。
- [ ] `scheduler-integration.test.ts` 不再输出 `musicbrainz.org` 或其他真实外网请求失败日志。
- [ ] `generate-now` 成功后 Brief 进入 `completed`，失败后进入可诊断终态；scheduler 路径同样同步 Brief 终态。
- [ ] 用户流级测试覆盖 Brief -> Plan -> Generate now -> completed Brief -> ready Project -> export fast-fail / export success 的状态链。
- [ ] 浏览器验收 blocker 被解决或保留为明确 HITL blocker，不能在未验收时标记 Phase 4 完成。
- [ ] 第一批 issue 的 `Status` 与 `AUTOMATION_STATE.md` 保持一致。
...
4. 修复 dev server 启动方式或记录需要用户本地完成的 HITL 验收。
